...
const AssetInquiry = ({ assets, onEdit, onDelete }: AssetInquiryProps) => {
  ...
  return (
    <div>
      ...
      <div className="flex justify-center items-center space-x-2 mb-6 p-4 bg-gray-50 rounded-lg">
        <input type="number" name="year" placeholder="년(YYYY)" onChange={handleFilterChange} className="p-2 border rounded-md w-24"/>
        <input type="number" name="month" placeholder="월" onChange={handleFilterChange} className="p-2 border rounded-md w-20"/>
        <input type="number" name="day" placeholder="일" onChange={handleFilterChange} className="p-2 border rounded-md w-20"/>
      </div>
          ...
          <tbody>
            {filteredAssets.map(asset => (
              <tr key={asset.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3">
                  <button onClick={() => onEdit(asset)} className="bg-yellow-500 text-white px-3 py-1 rounded-md mr-2 hover:bg-yellow-600">수정</button>
                  <button onClick={() => onDelete(asset.id)} className="bg-red-500 text-white px-3 py-1 rounded-md hover:bg-red-600">삭제</button>
                </td>
              </tr>
            ))}
          </tbody>
